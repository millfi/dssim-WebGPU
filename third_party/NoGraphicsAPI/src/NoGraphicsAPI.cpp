...
DeviceInit create_device(const DeviceDesc& desc) noexcept
{
    ...
    if (desc.vulkan && desc.vulkan->configure) {
        error = error_from_vk(desc.vulkan->configure(desc.vulkan->context, state->physical_device, device_info));
        if (error != Error::none)
            return fail_device_creation(state, error);
    }
...
void submit_commands(Span<CommandBuffer* const> commands,
                     ...
                     Span<const VkSemaphoreSubmitInfo> native_waits = {},
                     Span<const VkSemaphoreSubmitInfo> native_signals = {}) noexcept
{
    ...
    if (wait_semaphore) waits.push_back(wait_info);
    for (size_t i = 0; i < native_waits.size; ++i) waits.push_back(native_waits.data[i]);
    signals.push_back(work_signal_infos[0]);
    if (signal_semaphore) signals.push_back(work_signal_infos[1]);
    for (size_t i = 0; i < native_signals.size; ++i) signals.push_back(native_signals.data[i]);
...
VulkanDevice get_vulkan_device(Device* device) noexcept {
    return {device->instance, device->physical_device, device->device, device->queue, device->queue_family};
}
...
void submit_vulkan(Span<CommandBuffer* const> commands, TimelinePoint completion,
    ...
    assert(commands.data && commands.size);
    submit_commands(commands, commands.data[0]->state, completion,
                    VK_NULL_HANDLE, VK_NULL_HANDLE, waits, signals);
}
} // namespace gpu

namespace gpu {
void discard_vulkan_commands(CommandBuffer* commands) noexcept {
    assert(commands && commands->recording && !commands->swapchain);
    assert(!commands->pending_texture_initializations.first);
    auto* state = commands->state;
    auto* context = commands->context;
    require_vk(vkResetCommandPool(state->device, context->command_pool, 0));
    context->active = false;
    context->retire_value = 0;
    assert(state->active_command_buffers);
    --state->active_command_buffers;
    *commands = {};
}
} // namespace gpu
